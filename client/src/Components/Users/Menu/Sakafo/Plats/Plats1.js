...
//         </Link>
//         <p style={{ marginLeft: "40%", fontSize: "2rem", color: "maroon", fontFamily: "cursive", marginBottom:'2%'}}>LES PLATS DISPONIBLE</p>
//       </div>
...
function Plats1() {
  ...
  const handleAddToCart = (plats) => {
    dispatch(addToCart(plats)); // Dispatch the action to add item to cart
  };
  ...
  return (
    <div className='Dessertsglobal1'>
      ...
      <div>
        ...
        <p style={{ marginLeft: "40%", fontSize: "2rem", color: "#bf5700", fontFamily: "cursive", marginBottom: '2%' }}>LES PLATS DISPONIBLE</p>
      </div>
          ...
          {dt.map((Plats, index) => (
            <div key={index} className='Boissonsglobal' style={{ margin: "1%", width: "calc(25% - 2%)" }}>
              <div className="card w-96 bg-base-100 shadow-xl" style={{ width: "100%" }}>
                ...
                <div className="card-body text-center">
                  <h2 className="card-title" style={{ color: "green", marginLeft: '25%' }}>{Plats.nom}</h2>
                  <p style={{ marginLeft: "-10px" }}>
                    {Plats.prix} Ar<br />
                    {Plats.description}
                  </p>
                  <div className='Etoile'><ImStarFull /><ImStarFull /><ImStarFull /><ImStarFull /><ImStarEmpty /></div>
                  <div className="card-actions justify-center">
                    <button className="buttonsakafo" onClick={() => handleAddToCart(Plats)}>Buy Now</button>
                  </div>
                </div>
              </div>
            </div>
